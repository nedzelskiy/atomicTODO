...
class ThemesStylesCreatorPlugin {
  ...
  apply(compiler) {
    compiler.hooks.done.tap(PLUGIN_NAME, (stats) => {
      try {
        ...
        console.log('cachedStylesKeys', cachedStylesKeys)
        ...
        if (this.options.join) {
          ...
          this.createAndDropThemesFilesForEachFile();
        }
  ...
  createAndDropThemesFilesForEachFile() {
    const buildFolder = this.options.output || this.stats.compilation.outputOptions.path;
    Object.keys(this.readyStyleFiles).forEach((themeName) => {
      fse.outputFileSync(`${buildFolder}${themeName}`, JSON.stringify(cssToObject(this.readyStyleFiles[themeName].join('')), null, 4));
      ThemesStylesCreatorPlugin.consoleMessage('info', `${PLUGIN_NAME}: created theme "${themeName}"!`);
    });
  }
}
